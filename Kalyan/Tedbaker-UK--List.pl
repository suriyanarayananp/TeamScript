...
while($content=~m/<a\s*href="([^>]*?)\"\s*class\=\"categories_main\"[^>]*?>([^>]*?)<\/a>([\w\W]*?)\s*<a\s*class\=\"nav_header\s*nav/igs)
{
	my $url_1=$1;  
	...
	# Pattern match to get the Sub-topmenu url and menus.(menu2=>New Arrival,Clothing).
	while($main_block=~m/<a[^>]*?href\s*\=\s*(?:\"|\')([^>]*?)(?:\"|\')[^>]*?>\s*([^>]*?)\s*<\/a>\s*<\/span>([\w\W]*?)<\/ul>/igs) 
	{
		my $menu_2_url = $1;
		my $menu_2 = $utilityobject->Trim($2);   
		my $menu_2_block=$3; 
		$menu_2_url='http://www.tedbaker.com'.$menu_2_url unless($menu_2_url=~m/^\s*http\:/is);	
		
		# Getting the content of the menu3 url.
		my $menu_2_url_content = $utilityobject->Lwp_Get($menu_2_url); 
		
		# Pattern match to get each category block from main category block (eg.choose from).
		while($menu_2_url_content=~m/<option\s*value\s*\=\s*\"\s*\"\s*disabled\s*selected>(?![^<]*?Sizes|[^<]*?Prices)([^<]*?)<([\w\W]*?)<\/li>/igs) 
		{
			...
			{
				$category_url="$menu_2_url"."?q="."$1";
				my $category= $utilityobject->Trim($2); # Category name.
				$category =~s/Â//igs;
				$category_url=~s/f\:/f%3A/igs;
				$category_url=~s/\,/%2C/igs;	
				&get_Product($category_url,$menu_1,$menu_2,'','',$category_head,$category); #Function call with arguments category URLs and their corresponding menus.
			}
		}
				
		# Pattern match to get menu3 & it's url.(eg.menu3=>new arrivals,shirts).
		while($menu_2_block=~m/<a[^>]*?href="\s*([^>]*?)\s*\"\s*title="\s*[^>]*?\s*"\s*>\s*([^>]*?)\s*<\/a>/igs)
		{
			my $menu_3_link=$1;
			my $menu_3= $utilityobject->Trim($2);   
			
			next if($menu_3=~m/Home/is);
