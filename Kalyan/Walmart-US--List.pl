...
if($content=~m/(\s*<a[^>]*?href\=[^>]*?>\s*$ARGV[1]\s*[\w\W]*?)<\/a>\s*<\/div>\s*<\/div>/is)
{
	print "I am here\n";
	my $block1 = $1;
	while($block1 =~ m/href\=\"\s*[^>]*?\s*\"\s*>\s*([^>]*?)\s*<i>([\w\W]*?)<\/li>\s*<\/ul>/igs)
	{
		my $menu2 = $1;
		my $block2 = $2;
		$menu2 =~s/\&nbsp\;//igs;
		...
		while($block2 =~ m/href\=\"\s*([^>]*?)\s*\"\s*>\s*([^>]*?)\s*<\/a>/igs)
		{
			...
			#next unless($menu3 =~ m/Toddler/is);
			...
			# close ss;
			# exit;
			#next unless(($menu3=~m/backpacks/is)&&($menu2=~m/bags/is));
			##next unless(($menu3=~m/wedding/is)&&($menu2=~m/jewelry/is));			
			...
			elsif($cont2 =~ m/class\=(?:\")?expander\-toggle[^>]*?>\s*([^>]*?)\s*<\/a>([\w\W]*?)<\/div>\s*<\/div>\s*<\/div>/is)
			{
				print "Match2\n";
				while($cont2 =~ m/class\=(?:\")?expander\-toggle[^>]*?>\s*([^>]*?)\s*<\/a>([\w\W]*?<\/div>\s*)<\/div>\s*<\/div>/igs)  ## LH Filter Block
				{
					my $filterheader = $utilityobject->Decode($1);
					my $filterblock = $2;
					# print "$filterblock";
					print "hre2 :: $filterheader\n";
					#next unless($filterheader =~ m/popular\s*in\s*Baby/is);
					next if($filterheader =~ m/Department|rating|size|brand|price|save/is);  #Skip few filters
					
					# if($filterblock =~ m/href\=\"([^>]*?)\">\s*(?:<[^>]*?>\s*)\s*([^>]*?)\s*<\/span>|href\=\"([^>]*?)\">\s*(?:<[^>]*?>\s*)[\w\W]*?>([^>]*?)\s*<\/div>|href\=\"\s*([^>]*?)\"[^>]*?>\s*([^>]*?)\s*<\/a>/is)
					if($filterblock =~ m/href\=\"([^>]*?)\"[^>]*?data\-index\=\d+>\s*<span>\s*([^>]*?)\s*<\/span>|href\=\"([^>]*?)\"[^>]*?data\-name[^>]*?>\s*([^>]*?)\s*<\/a>/is)
					{
						...
						# while($filterblock =~ m/href\=\"([^>]*?)\">\s*(?:<[^>]*?>\s*)\s*([^>]*?)\s*<\/span>|href\=\"([^>]*?)\">\s*(?:<[^>]*?>\s*)[\w\W]*?>([^>]*?)\s*<\/div>|href\=\"\s*([^>]*?)\"[^>]*?>\s*([^>]*?)\s*<\/a>/igs)
						while($filterblock =~ m/href\=\"([^>]*?)\"[^>]*?data\-index\=\d+>\s*<span>\s*([^>]*?)\s*<\/span>|href\=\"([^>]*?)\"[^>]*?data\-name[^>]*?>\s*([^>]*?)\s*<\/a>/igs)
						{
							...
							# next if($filterheader =~ m/shop\s*by\s*category/is);
							if($filterheader =~ m/\bRetailer\b/is)
							{
								next unless($filtervalue=~ m/walmart/is);
							}
