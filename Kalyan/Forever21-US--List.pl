...
foreach my $regex(@regex_array)
{
	...
	if ( $content =~m/$regex/is )
	{
		...
		elsif((($menu_1=~m/$robo_menu/is)&&($robo_menu=~m/SALE\s*(?:&(?:amp\;)?\s*DEALS)?/is))||(($menu_1=~m/$robo_menu/is)&&($robo_menu=~m/SALE/is)))  # Pattern match to check whether Menu1 is Sale & Deals.
		{
		...
		elsif($menu_1=~m/$robo_menu/is)  # Pattern match to collect products under menu1=>"Other Than Women, Sale, New arrivals".
		{
			...
			while($content1=~m/(?:<start>\s*([^<]*?)\s*<[\w\W]*?)?<dt[^>]*?>\s*<a\s*href\=\"([^\"]*?)\"\s*[^>]*?>\s*<font\s*class\=\"SubCateg([^>]*?)\">\s*([\w\W]*?)<\/font>\s*<\/a>\s*<\/\s*dt>/igs) 
			{
				...
				my $menu_2=$4;      # Menu2.
				print "Menu 2 category:: $menu_2_cat\n";
				...
				if(($content2=~m/class\s*\=\s*\"\s*items_name\s*\"\s*>/is)&&(($menu_2!~m/Style\s*Deals/is)))
				{
