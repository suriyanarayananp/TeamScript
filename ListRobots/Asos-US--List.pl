	...
	while($top_menu_block =~ m/<dt>\s*($ARGV[1])\s*<\/dt>\s*<dd>\s*<ul\s*class\=\"items\">\s*([\w\W]*?)\s*\^\$\^/igs)
	{
		...
		while($menu_2_block =~ m/<a\s*href\=\"([^>]*?)\"[^>]*?>\s*([^>]*?)\s*<\/a>/igs)
		{
			...
			else
			{
				...
				while($menu_3_page =~ m/<h2\s*class\=\"title\">\s*((?!shop\s*by\s*price)[^>]*?)\s*<\/h2>\s*(?!\s*\-\-\>)([\w\W]*?)\s*<\/ul>/igs)
				{
...
sub Product_Insert()
{
	...
	{
		my $product_url = $1;
		$product_url = 'http://us.asos.com/pgeproduct.aspx'.$1 if($product_url =~ m/(\?iid\=\d+)/is);
