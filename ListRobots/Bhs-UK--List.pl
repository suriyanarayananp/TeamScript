...
my @regex_array=('<li\s*class=\"[^<]*?\">\s*<a\s*[^<]*\s*href\s*\=\s*\"([^<]*home)\"[^<]*title=\"(Sale\s*(?:&amp;\s*Offers)?|Offers)\"[^<]*>','<li\s*class=\"[^<]*?\">\s*<a\s*[^<]*\s*href\s*\=\s*\"([^<]*home)\s*\"[^<]*title=\"(Women)\"[^<]*>','<li\s*class=\"[^<]*?\">\s*<a\s*[^<]*\s*href\s*\=\s*\"([^<]*home)\s*\"[^<]*title\s*\=\s*\"\s*(Home\s*(?:\,?\s*Lighting\s*&(?:amp\;)?\s* Furnitures?)?\s*)\s*\"[^<]*>','<li\s*class=\"[^<]*?\">\s*<a\s*[^<]*\s*href\s*\=\s*\"([^<]*home)\s*\"[^<]*title=\"(Men)\"[^<]*>','<li\s*class=\"[^<]*?\">\s*<a\s*[^<]*\s*href\s*\=\s*\"([^<]*home)\"[^<]*title=\"(Boys)\"[^<]*>','<li\s*class=\"[^<]*?471109\"\s*>\s*<a\s*[^<]*\s*href\s*\=\s*\"([^<]*home)\"[^<]*title=\"(Wedding)\"[^<]*>','<li\s*class=\"[^<]*?\">\s*<a\s*[^<]*\s*href\s*\=\s*\"([^<]*home)\"[^<]*title=\"(Girls)\"[^<]*>','<li\s*class\s*\=\s*\"\s*[^<]*?\s*\"\s*>\s*<a[^>]*?href="\s*([^>]*?home)\s*\"[^>]*?>\s*(Holiday\s*Shop)\s*<');   
...
{	
	...
	if ( $content =~ m/$regex/is )
	{
		...
		else
		{
			...
			if(($menu_11=~m/$robo_menu/is)&&($menu_11=~m/(?:Home|women|Men|Boys|Girls|Wedding|Holiday\s*Shop)/is)) # Pattern matches to check whether menu1 is "Home or women or Men or children or Wedding" (Scenario 2).
			{
