...
sub Netaporter_UK_DetailProcess()
{
	...
	$robotname='Net-a-porter-UK--Detail';
	...
	{		
		...
		if($source_page=~m/productID\s*\=\s*\"?\s*(\d+)\s*(?:\;|\,|\")/is)#productID\s*\=\s*\"(\d+)\"\;
		{ 
		...
		if($source_page=~m/<span\s*class\=\"was\">\s*([^<]*?)\s*<\/span>\s*<span\s*class\=\"now\">\s*([^<]*?)\s*<\/span>\s*<span\s*class\=\"percentage\">\s*([^>]*?)\s*<\/span>/is){
			$price_text=$1.' '.$2.' '.$3;
			$current_price=$2;
		}
		...
		elsif($source_page=~m/<span\s*itemprop\=\"price\">\s*([^>]*?)\s*<\/span>/is){
