...
my $home_url='http://www.peacocks.co.uk/'; # RETAILER HOME URL : PEACOCKS, UK
my $source_page=&get_source_page($home_url);

my %validate;

# EXTRACTING TOP MENU/URL -> NEW IN, WOMENS, MENS, BOYS, GIRLS AND SALE - TRENDS (NOT REQUIRED)
while($source_page=~m/<a\s*href\=\"[^>]*?\"\s*class\=\"level\-top\"\s*>\s*<span>\s*([^>]*?)\s*<\/span>\s*([\w\W]*?)\s*<\!\-\-\s*\/menu\s*\-\->/igs){
	my $top_menu=&DBIL::Trim($1); # WOMENS
	my $top_menu_block=$2;
	
	if($top_menu=~m/New\s*in|Sale/is){ # ENTRY FOR TOP MENU : NEW IN AND SALE ONLY
		
		# EXTRACTING MENU 2
		while($top_menu_block=~m/<a\s*href\=\"([^>]*?)\"\s*>\s*<span>\s*([^>]*?)\s*<\/span>/igs){
			my $menu_2_url=$1;
			my $menu_2=&DBIL::Trim($2); # NEW WOMENS
			my $no_filter_url=$menu_2_url;
			$no_filter_url=$1.'/where/limit/all'.$2 if($no_filter_url=~m/([^>]*?)(\.html)/is); # DISPLAYS ALL PRODUCTS IN ONE PAGE
			my $no_filter_page=&get_source_page($no_filter_url); # PAGE SHOWS FULL PRODUCTS FOR THE CATEGORIES WITHOUT APPLYING FILTERS
			...
			while($menu_2_page=~m/<h4>\s*((?!price|size|offer|gender)[^>]*?)\s*<\/h4>([\w\W]*?)\s*<\/div>\s*<\/div>/igs){
				...
				while($filter_block=~m/setLocation\(\'([^>]*?)\'\)[^>]*?>\s*([^>]*?)\s*<span>/igs){
					...
					$filter_url=~s/where/where\/limit\/all/igs if($filter_url!~m/where\/limit\/all/is); # DISPLAYS ALL PRODUCTS IN ONE PAGE
					my $filter_page=&get_source_page($filter_url); # PAGE SHOWS FULL PRODUCTS AFTER APPLYING FILTERS
	...
	else{ # ENTRY FOR TOP MENU : WOMENS, MENS, BOYS AND GIRLS
		while($top_menu_block=~m/<h6>\s*<span>\s*([^>]*?)\s*<\/span>\s*<\/h6>([\w\W]*?)(?:<li\s*class\=\"level1[^>]*?>|<\!\-\-\s*\/cols\s*\-\->)/igs){
			my $menu_2=&DBIL::Trim($1); # SHOP DEPARTMENT
			my $menu_2_block=$2;
...
$dbh->disconnect;
# system(`/opt/home/merit/perl5/perlbrew/perls/perl-5.14.4/bin/perl /opt/home/merit/Merit_Robots/Peacocks-UK--Detail.pl  &`);
...
sub Product_Insert(){ # INSERT TAGS INTO DB
	...
	while($page=~m/<h2>\s*<a\s*href\=\"([^>]*?)\"\s*title\=\"[^>]*?\">/igs){
		my $product_url=$1;
		my $product_object_key;
		if($validate{$product_url} eq ''){ # CHECKING WHETHER PRODUCT URL ALREADY AVAILABLE IN THE HASH TABLE
			$product_object_key=&DBIL::SaveProduct($product_url,$dbh,$robotname,$retailer_id,$Retailer_Random_String,$excuetionid); # GENERATING UNIQUE PRODUCT OBJECT KEY
