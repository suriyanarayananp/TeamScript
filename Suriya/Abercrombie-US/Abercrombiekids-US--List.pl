...
my $content = lwp_get("http://www.abercrombiekids.com");

while($content =~ m/href\=\"([^>]*?)\">\s*<h2[^>]*?nav\-description\">\s*([^>]*?)\s*<\/h2>/igs)
{
	my $caturl = $1;
	my $menu1 = &clean($2);
	$caturl = "http://www.abercrombiekids.com".$caturl unless($caturl =~ m/^http/is);
	...
	if($subcontent =~ m/<ul\s*class\=\"primary\">([\w\W]*?)<\/ul>/is)
	{
		...
		while($subcont =~ m/href\=\"([^>]*?)\"\s*>\s*([^>]*?)\s*<\/a>/igs)
		{
			...
			$caturl2 = "http://www.abercrombiekids.com".$caturl2 unless($caturl2 =~ m/^http/is);
			...
			if($menu2 =~ m/Sale|Clearance/is)
			{
				...
				if($subcontent2 =~ m/>\s*$menu2\s*<\/a>\s*<ul\s*class\=\"secondary\">([\w\W]*?)\s*(?:<\/ul>\s*<\/div>|<li\s*id\=\"cat\-anf_division)/is)
				{
					...
					while($subcont2 =~ m/End:\s*Filter\s*Ability to Block Sub Categories\s*\-\->\s*<li[^>]*?>\s*<a\s*href\=\"([^>]*?)\">\s*([^>]*?)\s*<\/a>/igs)
					{
						my $caturl3 = $1;
						my $menu3 = &clean($2);
						$caturl3 = "http://www.abercrombiekids.com".$caturl3 unless($caturl3 =~ m/^http/is);
						...
						if($menucontent3=~m/class\=\"current\s*selected[^>]*?>\s*<[^>]*?>\s*$menu3\s*<\/a>\s*<ul([\w\W]*?)<\/ul>\s*<\/li>/is)
						{
							...
							while($block =~ m/href\=\"([^>]*?)\">\s*([^>]*?)\s*</igs)
							{
								my $menuurl_1 = $1;
								my $menu4 = &clean($2);
								$menuurl_1 = "http://www.abercrombiekids.com".$menuurl_1 unless($menuurl_1 =~ m/^http/is);
								my $menucontent_1 = lwp_get($menuurl_1);
								while($menucontent_1 =~ m/<h2>\s*<a\s*href\=\"([^>]*?)\"\s*data\-productId\=\"([^>]*?)\"/igs)
								{
									my $purl = $1;
									my $productid = $2;
									$purl = "http://www.abercrombiekids.com".$purl unless($purl =~ m/^http/is);
									my $product_object_key;
									if(defined $totalHash{$productid})
									{
										print "Data Exists! -> $totalHash{$productid}\n";
